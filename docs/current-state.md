...
- A first server/client/web split foundation is in place: documented split
  ...
  shared LAN media/subtitle streaming, settings-file root readback, and
  file-backed playback progress plus stable pairing-token persistence under
  the locked data directory. Headless
  hosts also announce themselves through the existing LAN discovery
