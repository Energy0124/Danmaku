...
- `[~]` Add first-pass localization support for English and Traditional Chinese
  ...
  chrome strings plus Home/Library placeholder/count, Home dashboard body, and
  Library workspace navigation/filter strings are backed by Compose
  Multiplatform generated resources under `commonMain/composeResources`.
...
- `[ ]` Continue migrating desktop `DesktopStrings` into Compose Multiplatform
  resources by feature slice, continuing with remaining Library inspector/import
