...
render = function()
    ...
    local seek_x = 92
    local seek_w = math.max(220, width - 184)
    local seek_y = bottom_y + 34
    local progress = 0
    if duration > 0 then
        progress = math.max(0, math.min(1, position / duration))
    end

    parts[#parts + 1] = rect(0, 0, width, TOP_HEIGHT, "000000", "98")
    parts[#parts + 1] = rect(0, bottom_y, width, BOTTOM_HEIGHT, "000000", "88")
    parts[#parts + 1] = text(24, 16, truncate(title, 80), 26, "FFFFFF", 7, true)
    parts[#parts + 1] = text(width - 24, 18, status, 22, "BFBFBF", 9, false)
    parts[#parts + 1] = text(24, bottom_y + 25, fmt_time(position), 23, "FFFFFF", 7, false)
    parts[#parts + 1] = text(width - 24, bottom_y + 25, duration > 0 and fmt_time(duration) or "--:--", 23, "FFFFFF", 9, false)
    ...
    local y = bottom_y + BUTTON_Y_OFFSET
    local x = 24
