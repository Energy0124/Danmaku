...
param(
    ...
    [string]$OutputDir,
    [string]$Tv1080pAvd = "Danmaku_TV_API_36",
    [string]$Tv4kAvd = "Danmaku_TV_4K_API_36",
...
if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $OutputDir = Join-Path $repoRoot "build\qa\android-tv"
}
...
$rustReportPath = Join-Path $OutputDir "android-tv-rust-server-qa.md"
$rustHostMode = "n/a"
$rustCatalogItems = 0
...
function Invoke-RustServerConnectivityTestForSerial {
    ...
    $testClass = "app.danmaku.tv.RustServerParityInstrumentedTest"
    $gradleArgs = @(
        "--no-daemon",
...
function Write-RustServerReport {
    ...
    $report = @(
        "# Android TV Rust Server Emulator QA",
        "",
...
try {
    ...
    Invoke-TvQaForAvd -AvdName $Tv1080pAvd -ScreenshotName "danmaku-tv-1080p"
    Invoke-TvQaForAvd -AvdName $Tv4kAvd -ScreenshotName "danmaku-tv-4k"
...
Write-Host "Android TV emulator QA complete. Output: $OutputDir"
